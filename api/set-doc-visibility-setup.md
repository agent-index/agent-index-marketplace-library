---
name: set-doc-visibility-setup
type: setup
version: 1.0.0
collection: library
description: Setup for the set-doc-visibility capability — minimal member-level setup; all configuration is org-mandated at collection install time.
target: set-doc-visibility
target_type: task
...
Set Document Visibility requires no member-specific configuration. All parameters are org-mandated and set during collection setup. This setup validates that the collection is installed and the member can reach the shared filesystem.
...
3. Register entry in `member-index.json` with alias `@ai:set-doc-visibility`.
4. Confirm to member: "Library is ready — say '@ai:set-doc-visibility' or just ask in plain language."
