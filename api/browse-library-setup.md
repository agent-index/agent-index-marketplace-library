---
name: browse-library-setup
type: setup
version: 1.0.0
collection: library
description: Setup for the browse-library capability — minimal member-level setup; all configuration is org-mandated at collection install time.
target: browse-library
target_type: skill
...
Browse Library requires no member-specific configuration. All parameters are org-mandated and set during collection setup. This setup validates that the collection is installed and the member can reach the shared filesystem.
...
3. Register entry in `member-index.json` with alias `@ai:browse-library`.
4. Confirm to member: "Library is ready — say '@ai:browse-library' or just ask in plain language."
