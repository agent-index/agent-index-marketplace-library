---
name: library-tutorial-setup
type: setup
version: 1.0.0
collection: library
description: Setup for the library-tutorial capability — minimal member-level setup; all configuration is org-mandated at collection install time.
...
Library Tutorial requires no member-specific configuration. All parameters are org-mandated and set during collection setup. This setup validates that the collection is installed and the member can reach the shared filesystem.
...
3. Register entry in `member-index.json` with alias `@ai:library-tutorial`.
4. Confirm to member: "Library is ready — say '@ai:library-tutorial' or just ask in plain language."
