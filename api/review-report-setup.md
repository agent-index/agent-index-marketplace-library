---
name: review-report-setup
type: setup
version: 1.0.0
collection: library
description: Setup for the review-report capability — minimal member-level setup; all configuration is org-mandated at collection install time.
...
Review Report requires no member-specific configuration. All parameters are org-mandated and set during collection setup. This setup validates that the collection is installed and the member can reach the shared filesystem.
...
3. Register entry in `member-index.json` with alias `@ai:review-report`.
4. Confirm to member: "Library is ready — say '@ai:review-report' or just ask in plain language."
